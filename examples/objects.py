...
class Person:
    ...
    def add_address(self, street, city, country):
        """Ajouter une adresse."""
        self.addresses.append({"street": street, "city": city, "country": country})
...
def main():
    ...
    print(
        f"Entreprise: {data['company'].name} (fondée en {data['company'].founded_year})"
    )
    ...
    print(
        f"  Employé 1: {data['company'].employees[0].name} ({data['company'].employees[0].age} ans)"
    )
    ...
    print(f"  Départements: {list(data['company'].departments.keys())}")
    print(
        f"  Budget Engineering: ${data['company'].departments['Engineering']['budget']:,}"
    )
