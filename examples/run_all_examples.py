...
class ExampleRunner:
    ...
    def run_example(self, example_file: Path) -> Dict:
        ...
        try:
            ...
            with redirect_stdout(output_buffer), redirect_stderr(error_buffer):
                # Exécuter le module
                exec(open(example_file, 'r', encoding='utf-8').read(), {
                    '__name__': '__main__',
                    '__file__': str(example_file)
                })
            ...
            result["changelog_entries"] = self._count_changelog_entries(result["output"])
    ...
    def _analyze_features(self, output: str) -> List[str]:
        ...
        if "added" in output_lower or "edited" in output_lower or "removed" in output_lower:
    ...
    def run_all_examples(self) -> None:
        ...
        example_files = [
            f for f in self.examples_dir.glob("*.py")
    ...
    def generate_report(self) -> None:
        ...
        for result in self.results:
            ...
            if result["status"] == "success":
                if result["changelog_entries"] > 0:
                    print(f"      📝 {result['changelog_entries']} entrée(s) de changelog")
        ...
        if any("Context Manager Global" in r["features_demonstrated"] for r in self.results):
